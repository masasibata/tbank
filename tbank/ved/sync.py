...
class VedClient(BaseSyncClient):
    """Синхронный клиент ВЭД (валютный контроль): постановка контракта на учёт,
    ...
    def _signed_post(self, path: str, request: BaseModel, parser: Type[T]) -> T:
        ...
        response = transport.request("POST", path, content=body, headers=headers)
        self._raise_for_http(response)
        return parser.model_validate(self._parse_body(response))

    def register_contract(self, request: RegisterContractRequest) -> ApplicationResult:
        """Поставить валютный контракт на учёт (mTLS + подпись)."""
        return self._signed_post(_endpoints.REGISTRATION, request, ApplicationResult)

    def amend_contract(self, request: AmendContractRequest) -> ApplicationResult:
        """Внести изменения в валютный контракт (mTLS + подпись)."""
        return self._signed_post(_endpoints.AMENDMENT, request, ApplicationResult)

    def deregister_contract(
        self, request: DeregisterContractRequest
    ) -> ApplicationResult:
        """Снять валютный контракт с учёта (mTLS + подпись)."""
        return self._signed_post(_endpoints.DEREGISTRATION, request, ApplicationResult)

    def get_application_status(self, application_id: str) -> ApplicationStatusInfo:
