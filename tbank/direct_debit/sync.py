...
class DirectDebitClient(BaseSyncClient):
    ...
    def get_rule(self, rule_id: str) -> RuleDetails:
        """Карточка правила."""
        return self._get(f"{_V1}/rules/{rule_id}", _RULE_DETAILS)

    def update_rule(self, rule_id: str, rule: RuleUpdate) -> RuleDetails:
        """Обновить правило."""
        return self._send("PUT", f"{_V1}/rules/{rule_id}", _RULE_DETAILS, body=rule)

    def delete_rule(self, rule_id: str) -> None:
        """Удалить правило."""
        self._send("DELETE", f"{_V1}/rules/{rule_id}")
    ...
    def get_payment_request(self, request_id: str) -> PaymentRequestDetails:
        """Карточка платёжного требования."""
        return self._get(f"{_V1}/requests/{request_id}", PaymentRequestDetails)

    def revoke_payment_request(
    ...
    def get_payment_request_file(self, request_id: str) -> PaymentRequestFile:
        """PDF платёжного требования (base64)."""
        return self._get(f"{_V1}/requests/{request_id}/file", PaymentRequestFile)
    ...
    def get_agreement(self, agreement_id: str) -> AgreementDetails:
        """Карточка соглашения."""
        return self._get(f"{_V1}/agreements/{agreement_id}", AgreementDetails)

    def get_agreement_file(self, agreement_id: str) -> AgreementFile:
        """PDF-файл соглашения (base64)."""
        return self._get(f"{_V1}/agreements/{agreement_id}/file", AgreementFile)
