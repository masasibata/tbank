...
class TaxConsultClient(BaseSyncClient):
    """Синхронный клиент налоговых консультаций: карточка заявки, чат с менеджером,
    ...
    def get_request(self, tax_request_id: str) -> ConsultRequest:
    ...
    def get_chat(self, tax_request_id: str, *, limit: int, offset: int) -> ChatPage:
    ...
    def send_message(self, tax_request_id: str, request: SendMessageRequest) -> None:
    ...
    def download_attachment(self, tax_request_id: str, attachment_id: str) -> bytes:
    ...
    def start_review(self, tax_request_id: str, cas_version: int) -> WorkflowState:
        """Взять заявку на рассмотрение."""
        return self._workflow("start-review", tax_request_id, cas_version)

    def start_work(self, tax_request_id: str, cas_version: int) -> WorkflowState:
        """Начать работу по заявке."""
        return self._workflow("start-work", tax_request_id, cas_version)

    def request_clarification(
        self, tax_request_id: str, cas_version: int
    ) -> WorkflowState:
        """Запросить уточнение у клиента."""
        return self._workflow("clarification", tax_request_id, cas_version)

    def decline(self, tax_request_id: str, cas_version: int) -> WorkflowState:
        """Отклонить заявку."""
        return self._workflow("decline", tax_request_id, cas_version)

    def cancel(self, tax_request_id: str, cas_version: int) -> WorkflowState:
        """Отменить заявку."""
        return self._workflow("cancel", tax_request_id, cas_version)

    def set_pending_payment(
        self, tax_request_id: str, cas_version: int
    ) -> WorkflowState:
        """Перевести заявку в ожидание оплаты."""
        return self._workflow("pending-payment", tax_request_id, cas_version)

    def confirm_payment(self, tax_request_id: str, cas_version: int) -> WorkflowState:
        """Подтвердить оплату заявки."""
        return self._workflow("payment-done", tax_request_id, cas_version)

    def mark_ready(self, tax_request_id: str, cas_version: int) -> WorkflowState:
        """Отметить заявку готовой (ответ предоставлен)."""
        return self._workflow("ready", tax_request_id, cas_version)
