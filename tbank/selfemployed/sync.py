...
class SelfEmployedClient(BaseSyncClient):
    """Синхронный клиент выплат самозанятым (e2c): анкеты, платёжные реестры,
    ...
    def _initiate(self, path: str, body: CorrelatedRequest, *, secured: bool) -> str:
        ...
        response = transport.request("POST", path, json=payload)
        self._raise_for_http(response)
        return body.correlation_id

    # --- Самозанятые (получатели) ---

    def create_recipients(self, request: CreateRecipientsRequest) -> str:
        """Создать черновики анкет самозанятых."""
        return self._initiate(_P_CREATE_RECIPIENTS, request, secured=False)

    def get_create_recipients_result(self, correlation_id: str) -> RecipientResults:
        """Результат создания черновиков анкет."""
        return self._call(
            _CREATE_RECIPIENTS_RESULT, CorrelationRef(correlation_id=correlation_id)
        )

    def add_recipients_by_requisites(
        self, request: AddRecipientsByRequisitesRequest
    ) -> str:
        """Добавить самозанятых по реквизитам."""
        return self._initiate(_P_ADD_RECIPIENTS, request, secured=False)

    def get_add_recipients_result(self, correlation_id: str) -> RecipientResults:
        """Результат добавления самозанятых по реквизитам."""
        return self._call(
            _ADD_RECIPIENTS_RESULT, CorrelationRef(correlation_id=correlation_id)
        )

    def list_recipients(self, request: ListRecipientsRequest) -> ListRecipientsResult:
        """Информация по самозанятым (фильтры + пагинация)."""
        return self._call(_LIST_RECIPIENTS, request)

    # --- Платёжный реестр ---

    def create_payment_registry(self, request: CreatePaymentRegistryRequest) -> str:
        """Создать черновик платёжного реестра."""
        return self._initiate(_P_CREATE_REGISTRY, request, secured=False)
    ...
    def get_submit_result(self, correlation_id: str) -> SubmitRegistryResult:
        """Результат подписания реестра (mTLS)."""
        return self._call(_SUBMIT_RESULT, CorrelationRef(correlation_id=correlation_id))

    def pay_payment_registry(
        self, registry_id: int, *, correlation_id: Optional[str] = None
    ) -> str:
    ...
    def get_pay_result(self, correlation_id: str) -> PayRegistryResult:
        """Результат оплаты реестра (mTLS)."""
        return self._call(_PAY_RESULT, CorrelationRef(correlation_id=correlation_id))
    ...
    def get_receipts_result(self, correlation_id: str) -> ReceiptsResult:
    ...
    def list_payment_registries(
        ...
        return self._call(_LIST_REGISTRIES, request)
